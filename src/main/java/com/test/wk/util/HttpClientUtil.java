...
public class HttpClientUtil {
    ...
    public static Map<String,String> httpGetMethodWithStatusCode(HttpConfig httpConfig) throws IOException {
    ...
    public static Map<String,String> httpDeleteMethodWithEntityWithStatusCode(HttpConfig httpConfig){
        ...
        try {
            ...
            statusCode = response.getStatusLine().getStatusCode();
            if (200 == statusCode || 201 == statusCode) {
                entityString = convertResponseToString(response,null);
            }
            result.put("code",statusCode.toString());
            result.put("message",entityString);
            logger.info("httpDeleteMethodWithStatusCode response is {}",result.toString());
    ...
    private static String convertResponseToString(HttpResponse response,String charset) throws IOException {
        return  EntityUtils.toString(response.getEntity(), StringUtils.isEmpty(charset)?"UTF-8":charset);
    }
